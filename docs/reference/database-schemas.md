...
| 11      | Singleton state tables folded into config_machine_state; write-only cron_store_epochs retired                                                                                                                                                    | Unreleased          |
...
Schema 11 folded small state snapshots into `config_machine_state` and retired the write-only cron store epoch table. A schema 10 build still expects the nine former tables, so a manual downgrade must recreate their exact schemas and indexes before lowering the version.
